...
unsigned char _WorkMemory[65536]={0};//MAIN Memory
unsigned char _IOMemory[256]={0};//I/O AREA

//配置アドレスは0x0000です
//BDOS(0005H)のfunction 2と9のみ実装
unsigned char BDOS_SYSTEM[256]={
	0xcd,0x00,0x01,//JP 0100h プログラム実行
	0xed,0x76,//ed HALT　強制停止
	0x79,0xfe,0x2,0x20,0x4,//LD A,C;CP 2 JR NZ,NEXT1
	0x7b,0xd3,0x98,0xc9,//LD A,E;OUT (0x98),A;RET
	0xfe,0x09,0xc0,//NEXT1:CP 9;RET NZ
	0x1a,0xfe,0x24,0xc8,//LOOP1:LD A,(DE);CP '$';RET Z
...
unsigned char VRAMVirtualAddress[256*3]={'H','E','L','L','O',' ','W','O','R','L','D',13,10,0};//仮想画面 SCREEN1 32*24
